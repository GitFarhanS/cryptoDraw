...
import { evaluateGraph } from './evaluate-graph.js'
import { serializeBytesToFormat } from '../converter-block/format-bytes.js'
import { upsertEdgeForInputPort } from './edge-types.js'

function edge(id, fromBlockId, fromPortKey, toBlockId, toPortKey) {
  return {
    id,
...
describe('evaluateGraph data transfer', () => {
  ...
  const result = evaluateGraph(blocks, edges)
  ...
  const rewired = upsertEdgeForInputPort([first], second)

  const result = evaluateGraph(blocks, rewired)
