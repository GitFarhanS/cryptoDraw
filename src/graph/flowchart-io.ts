...
export function serializeFlowchartToBase64(placedBlocks: PlacedBlockRecord[], edges: GraphEdge[]) {
    ...
    const compressed = deflateSync(new TextEncoder().encode(jsonText), {
        level: 9,
    });
...
function sanitizeBlocks(blocks: unknown[]) {
    ...
    return blocks.map((block, index) => {
        ...
        if (
            OP_DISPLAY_FORMATS.includes(
                candidate.opDisplayFormat as (typeof OP_DISPLAY_FORMATS)[number]
            )
        ) {
...
function bytesToUtf8(bytes: Uint8Array) {
    const BufferCtor = (
        globalThis as typeof globalThis & {
            Buffer?: {
                from(data: Uint8Array): { toString(encoding: 'utf8'): string };
            };
        }
    ).Buffer;
    if (BufferCtor) {
