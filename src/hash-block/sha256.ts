/** FIPS 180-4 SHA-256 and HMAC-SHA256 (synchronous, no WebCrypto). */
...
export function sha256(message: Uint8Array): Uint8Array {
    ...
    const ml = l * 8;
    const mlHi = Math.floor(ml / 0x1_0000_0000);
    const mlLo = ml >>> 0;
    ...
    dv.setUint32(totalLen - 8, mlHi, false);
    dv.setUint32(totalLen - 4, mlLo, false);
    ...
    for (let offset = 0; offset < totalLen; offset += 64) {
        ...
        for (let i = 16; i < 64; i++) {
            w[i] =
                (smallSigma1(w[i - 2]!) + w[i - 7]! + smallSigma0(w[i - 15]!) + w[i - 16]!) >>> 0;
        }
